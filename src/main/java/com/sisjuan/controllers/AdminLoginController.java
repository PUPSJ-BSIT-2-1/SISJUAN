...
import com.sisjuan.utilities.EmailService;
...
public class AdminLoginController {
    ...
    @FXML private void backToFrontPage() {
        ...
        try {
            u.loadStage(stage, "/com/sisjuan/fxml/GeneralFrontPage.fxml", StageAndSceneUtils.WindowSize.MEDIUM);
        } catch (IOException e) {
                    ...
                    if (isAuthenticated) {
                        ...
                        if (adminFullName != null) {
                            Platform.runLater(() -> {
                                ...
                                try {
                                    StageAndSceneUtils.loadStage(stage,"/com/sisjuan/fxml/AdminDashboard.fxml", StageAndSceneUtils.WindowSize.MEDIUM);
                                    ...
                                    if (stage.getScene() != null) {
                                        ...
                                        SISJUAN.applyThemeToSingleScene(stage.getScene(), darkModeEnabled);
                                    }
                                } catch (IOException e) {
                                    showAlert(
    ...
    private void applyInitialTheme() {
        if (mainLoginPane != null && mainLoginPane.getScene() != null) {
            Preferences userPrefs = Preferences.userNodeForPackage(GeneralSettingsController.class).node(USER_TYPE);
            boolean darkModeEnabled = userPrefs.getBoolean(GeneralSettingsController.THEME_PREF, false);
            SISJUAN.applyThemeToSingleScene(mainLoginPane.getScene(), darkModeEnabled);
        } else {
            Platform.runLater(() -> {
                if (mainLoginPane != null && mainLoginPane.getScene() != null) {
                    Preferences userPrefs = Preferences.userNodeForPackage(GeneralSettingsController.class).node(USER_TYPE);
